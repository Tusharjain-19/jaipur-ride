...
function goBack() {
  if (journeyManager.state !== JourneyState.IDLE) {
...
// ═══════════════════════════════════════
//  JOURNEY STATE & MANAGER (GPS-Only)
// ═══════════════════════════════════════
const JourneyState = {
  IDLE: "IDLE",
...
class JourneyManager {
  ...
  updateUI() {
    ...
    this.syncActiveJourneyBar();
  }

  syncActiveJourneyBar() {
    ...
    if (activeBarEl && overlayEl) {
      if (this.state !== JourneyState.IDLE && overlayEl.classList.contains("hidden")) {
        const activeSub = document.getElementById("active-journey-sub");
        if (activeSub && this.activeJourney) {
          ...
          activeSub.textContent = `${T("towards")} ${destName}`;
        }
  ...
  async startJourney(journey) {
    ...
    // 1. Stop any existing geolocation watch
    this.stopGps();

    // 2. Open overlay IMMEDIATELY (Navigation happens instantly, no GPS/API block)
    const overlay = document.getElementById("journey-overlay");
    if (overlay) {
      overlay.classList.remove("hidden");
    ...
    // Reset closest station text and nearby link
    ...
    if (nbLink) nbLink.classList.add("hidden");
    ...
    // Request permission immediately when starting live journey
    const perm = await checkAndRequestLocationPermission();
    console.log("[DEBUG] Location permission status for live journey:", perm);
    if (perm === 'granted') {
      // Hide compact location card, show route container, start GPS automatically in background
      document.getElementById("jv-location-card")?.classList.add("hidden");
      document.getElementById("jv-route-container")?.classList.remove("hidden");
      this.startWatchingGps();
    } else {
      // Show compact location card, hide route container
      document.getElementById("jv-location-card")?.classList.remove("hidden");
      document.getElementById("jv-route-container")?.classList.add("hidden");
    }

    // Wire compact location card enable button (for retry)
    const enableBtn = document.getElementById("enable-location-btn");
    if (enableBtn) {
      enableBtn.onclick = async (e) => {
        e.stopPropagation();
        const reqResult = await checkAndRequestLocationPermission();
  ...
  startWatchingGps() {
    ...
    if (closestEl) {
      closestEl.textContent = currentLang === 'hi'
        ? 'लोकेशन की खोज की जा रही है...'
        : 'Getting location...';
    }

    this.transitionTo(JourneyState.GPS_ACTIVE);
    ...
    try {
      this.watchId = nativeWatchPosition(
        ...
        (error) => {
          ...
          if (closestEl) {
            closestEl.textContent = currentLang === 'hi'
              ? 'जीपीएस सिग्नल कमजोर है'
              : 'Weak GPS signal...';
          }
        },
        { enableHighAccuracy: true, timeout: 10000, maximumAge: 5000 }
      );
      console.log("Location watcher active");
    } catch (e) {
      console.error("[JourneyManager] Failed to start watchPosition:", e);
      if (closestEl) {
        closestEl.textContent = currentLang === 'hi'
          ? 'स्थान सेवाएँ अनुपलब्ध हैं'
  ...
  processLocationUpdate() {
    ...
    const routeStations = this.activeJourney.parts.flatMap(p => p.stations);
    ...
    routeStations.forEach((s, idx) => {
      const d = getDistance(this.userLocation.lat, this.userLocation.lon, s.lat, s.lon);
      if (d < minDist) {
        minDist = d;
        nearestStn = s;
        nearestIdx = idx;
    ...
    if (!nearestStn) return;

    const distToStart = getDistance(this.userLocation.lat, this.userLocation.lon, routeStations[0].lat, routeStations[0].lon);
    const distToEnd = getDistance(this.userLocation.lat, this.userLocation.lon, routeStations[totalStations - 1].lat, routeStations[totalStations - 1].lon);

    let calculatedState = JourneyState.TRAVELLING;
    let statusText = "";

    const startName = T_STATION(routeStations[0].name);
    const endName = T_STATION(routeStations[totalStations - 1].name);
    const nearestName = T_STATION(nearestStn.name);
    ...
    if (nearestIdx === 0) {
      if (distToStart > 0.100) {
        calculatedState = JourneyState.WALKING_TO_STATION;
        statusText = currentLang === 'hi'
          ? `${startName} की ओर बढ़ रहे हैं (${distLabel} दूर)`
          : `Walking to ${startName} (${distLabel} away)`;
      } else {
        calculatedState = JourneyState.BOARDED_METRO;
        statusText = currentLang === 'hi'
          ? `${startName} पर हैं • प्लेटफॉर्म ${this.activeJourney.parts[0].startPlatform} पर चढ़ें`
          : `At ${startName} • Board at Platform ${this.activeJourney.parts[0].startPlatform}`;
      }
    } else if (nearestIdx === totalStations - 1) {
      if (distToEnd <= 0.050) {
        calculatedState = JourneyState.ARRIVED;
        statusText = currentLang === 'hi'
          ? `गंतव्य पर पहुंच गए: ${endName}`
          : `Arrived at destination: ${endName}`;
      } else {
        calculatedState = JourneyState.APPROACHING_DESTINATION;
        statusText = currentLang === 'hi'
          ? `${endName} के निकट (${distLabel} दूर)`
          : `Approaching ${endName} (${distLabel} away)`;
      }
    } else {
      if (distToEnd <= 0.500) {
        calculatedState = JourneyState.APPROACHING_DESTINATION;
      } else {
        calculatedState = JourneyState.TRAVELLING;
      }

      if (minDist <= 0.050) {
        statusText = currentLang === 'hi'
          ? `आप ${nearestName} मेट्रो स्टेशन पर हैं`
          : `You are at ${nearestName} Metro Station`;
      } else {
        statusText = currentLang === 'hi'
          ? `निकटतम स्टेशन: ${nearestName} (${distLabel} दूर)`
          : `Nearest Station: ${nearestName} (${distLabel} away)`;
      }
    }

    this.transitionTo(calculatedState);

    const closestEl = document.getElementById("jv-closest-station");
    if (closestEl && closestEl.textContent !== statusText) closestEl.textContent = statusText;

    updateRouteVisuals(routeStations, nearestIdx);

    const nbLink = document.getElementById("jv-nearby-link");
    if (nbLink) {
      const attrs = stationAttractions[nearestStn.name] || [];
      if (attrs.length > 0 && (nearestIdx === 0 || nearestIdx === totalStations - 1)) {
        const text = currentLang === 'hi' ? `आसपास देखें (${attrs.length})` : `See nearby (${attrs.length})`;
        if (nbLink.textContent !== text) nbLink.textContent = text;
        nbLink.classList.remove("hidden");
        nbLink.onclick = () => showNearbyPopup(nearestStn.name);
      } else {
...
function stopSim() {
  showModal({
    title: T("endJourneyTitle"),
    body: `<div style="text-align:center; padding:10px 0;"><p style="margin:0; font-size:14px; color:var(--text-sub);">${T("confirmFinishBody")}</p></div>`,
    confirmText: T("finishJourney"),
    cancelText: T("cancel"),
    onConfirm: () => {
      journeyManager.resetJourney();
      toast(T("journeyFinishedToast"));
    }
  });
}
    ...
    if (perm === 'prompt') {
      ...
      return new Promise((resolve) => {
        showModal({
          ...
          confirmText: currentLang === 'hi' ? 'अनुमेय' : 'Allow',
...
async function triggerRealLocationAccess() {
  ...
  try {
    ...
    console.log("Current GPS acquired");
    const lat = position.coords.latitude;
    const lon = position.coords.longitude;

    if (!metroData || !metroData.pink || !metroData.pink.stations) {
      throw new Error("Station data is unavailable");
    }
    ...
    if (nearest) {
      ...
      try {
        ...
        console.warn("Haptic impact failed:", hapticErr);
      }

      if (startDropdown) {
        startDropdown.select(nearest.id);
      ...
      setTimeout(() => {
        try {
          ...
          console.error("Failed to show navigation modal:", modalErr);
        }
      }, 500);
    } else {
    ...
    try {
      ...
      toast(currentLang === 'hi' ? 'स्थान प्राप्त करने में विफल' : 'Failed to acquire location');
    }
  }
}
...
function wire() {
  ...
  if (finishBtn) finishBtn.addEventListener("click", stopSim);
