...
export default function ExploreClient({ initialAttractions, language }: ExploreClientProps) {
  ...
  const types = useMemo(() => {
    const allTypes = initialAttractions.map((a) => a.type);
    return ["all", ...Array.from(new Set(allTypes))];
  }, [initialAttractions]);

  // Filter based on search query and type
  const filteredAttractions = useMemo(() => {
    return initialAttractions.filter((att) => {
      const matchesSearch =
  ...
  }, [searchQuery, selectedType, initialAttractions]);
            ...
            >
              ...
              <div className="p-6 space-y-4 flex-1 flex flex-col justify-between">
                <div className="space-y-3">
                  ...
                  <p className="text-xs text-foreground/75 leading-relaxed line-clamp-3 font-sans">
                    {isEn ? att.description : att.descriptionHi}
                  </p>
                </div>

                {/* Additional metadata */}
                <div className="space-y-3 pt-4 border-t border-light-border dark:border-white/5 text-xs font-sans">
                  ...
                  <div className="flex items-center space-x-2 text-foreground/75">
                    ...
                    <span>
                      ...
                      {att.walk_time_min
                        ...
                        : (isEn
                            ? `${att.approx_drive_distance_km} km (~${att.approx_drive_time_min} min drive)`
                            : `${att.approx_drive_distance_km} किमी (~${att.approx_drive_time_min} मिनट ड्राइव)`)}
                    </span>
                  </div>
                </div>
